...
Term BTOR2Encoder::bv_to_bool(const Term & t) const
{
  ...
  if (sort->get_sort_kind() == BV) {
    if (sort->get_width() != 1) {
      throw PanguException("Can't convert non-width 1 bitvector to bool.");
    }
    return solver_->make_term(
...
void BTOR2Encoder::preprocess(const std::string& filename) {
  ...
  if (!input_file) {
    throw PanguException("Could not open " + filename);
  }

  reader_ = btor2parser_new();

  if (!btor2parser_read_lines(reader_, input_file)) {
    throw PanguException(std::string(btor2parser_error(reader_)));
  }
  ...
  while ((l_ = btor2parser_iter_next(&it_))) {
    ...
    if (l_->tag == BTOR2_TAG_state) {
      ...
      if (l_->nargs == 0)
        throw PanguException("Missing term for id " + std::to_string(l_->id));
  ...
  while ((l_ = btor2parser_iter_next(&it_))) {
    ...
    for (i_ = 0; i_ < l_->nargs; i_++) {
      ...
      if (terms_.find(idx_) == terms_.end()) {
        throw PanguException("Missing term for id " + std::to_string(idx_));
      }
    ...
    if (l_->tag == BTOR2_TAG_state) {
      ...
      switch (l_->sort.tag) {
        ...
        default:
          ...
          // it's really bad
          throw PanguException("Unknown sort tag");
      }
      ...
      if (termargs_.size() != 2) {
        throw PanguException("Expecting two term arguments to init");
      ...
      if (linesort_->get_sort_kind() == BV) {
        ...
        throw PanguException("Unhandled sort: "
      ...
      if (termargs_.size() != 2) {
        throw PanguException("Expecting two arguments to next");
      }
      ...
      if (s0 == s1) {
        ...
        throw PanguException("Got two different sorts in next update.");
      }
      ...
      if (termargs_.size() != 2) {
        throw PanguException("Expecting two arguments to eq");
      }
      Term t0 = termargs_[0];
      Term t1 = termargs_[1];
      Sort s0 = t0->get_sort();
      ...
      if (termargs_.size() != 2) {
        throw PanguException("Expecting two arguments to neq");
      }
      Term t0 = termargs_[0];
      Term t1 = termargs_[1];
      Sort s0 = t0->get_sort();
      ...
      if (termargs_.size() != 2) {
        throw PanguException("Expecting two arguments to implies");
      }
      Term t0 = termargs_[0];
      Term t1 = termargs_[1];
      Sort s0 = t0->get_sort();
     ...
     ********************************/
      ...
      if (!termargs_.size()) {
        throw PanguException("Expecting non-zero number of terms");
      }
      ...
      if (boolopmap.find(l_->tag) != boolopmap.end()) {
        ...
        if (sk == BV) {
          ...
          throw PanguException("Unexpected sort");
        }
      } else {
    ...
    if (l_->symbol && l_->tag != BTOR2_TAG_input && l_->tag != BTOR2_TAG_output
      ...
      try {
        ...
        cout<< "BTOR2Encoder Warning: " <<e.what()<<endl;
      }
    }
