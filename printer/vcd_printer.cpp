    ...
    const smt::Term & ast)
{
  ...
  if (signal_set.find(short_name) != signal_set.end()) {
    ...
    throw PanguException(full_name + " has been registered already");
  }
...
void VCDWitnessPrinter::GenHeader(std::ostream & fout) const {
  fout << "$date" << std::endl;
  {
    ...
    if ( strftime(buffer, 100, date_time_format, timeinfo) == 0)
      throw PanguException("Bug: time2string conversion failed.");
...
void VCDWitnessPrinter::dump_all(const smt::UnorderedTermMap & valmap,
  ...
  for (auto && sig_bv_ptr : allsig_bv_) {
    ...
    if (pos == valmap.end()) {
	  ...
	  cout<<"missing value in provided trace @"<<t<<": "<<sig_bv_ptr->full_name<<endl;
      continue;
    }
  ...
  for (auto && sig_array_ptr : allsig_array_) {
    ...
    //    sig_array_ptr->full_name);
	  cout<<"missing value in provided trace @"<<t<<": "<<sig_array_ptr->full_name<<endl;
      continue;
    }
    ...
    while (memvalue->get_op() == smt::Store) {
      ...
      if (addr_pos != sig_array_ptr->indices2hash.end()) {
		...
		cout<<"missing addr index for array: "<<sig_array_ptr->full_name<<": , addr :"<<addr<<endl;
      }
...
//          sig_array_ptr->full_name, "-default-");
		cout<<"missing addr index for array: "<<sig_array_ptr->full_name<<": ,addr : "<<"-default-"<<endl;
      }
...
//        sig_bv_ptr->full_name, std::to_string(t));
      cout<<"Bug, "<<sig_bv_ptr->full_name<<" was not cached before time : "<<std::to_string(t)<<"."<<endl;
      continue;
    }
    ...
    while (memvalue->get_op() == smt::Store) { // peel the (store (store ...))
      ...
      if (addr_pos != sig_array_ptr->indices2hash.end()) {
        ...
        if (prev_pos == valprev.end()) {
		  ...
		  cout<<sig_array_ptr->full_name + "[" + addr + "]"
					<<" was not cached before time : "<<std::to_string(t)<<endl;
        } else {
      ...
      if (addr_pos != sig_array_ptr->indices2hash.end()) {
        ...
        if (prev_pos == valprev.end()) {
          ...
          //  sig_array_ptr->full_name+"[default]", std::to_string(t));
			cout<<sig_array_ptr->full_name+"[default]"
					<<" was not cached before time : "<<std::to_string(t)<<"."<<endl;
        } else {
        ...
        // logger.log(1, "missing addr index for array: {}: , addr : {}" ,
        //  sig_array_ptr->full_name, "-default-");
        cout<<"missing addr index for array: "<<sig_array_ptr->full_name
					<<": , addr : "<<"-default-"<<endl;
      }
    } // handling the inner constant default
  } // for all array signals
...
void VCDWitnessPrinter::DumpValues(std::ostream & fout) const {
  ...
  if (cex_.empty()) throw PanguException("No trace to dump");
    ...
    const std::string & vcd_file_name) const
{
  ...
  if (!fout.is_open())
    throw PanguException("Unable to write to : " + vcd_file_name);

  GenHeader(fout);
  DumpValues(fout);
  // logger.log(0, "Trace written to " + vcd_file_name);
  cout<<"Trace written to " + vcd_file_name<<endl;
}  // dump_trace_to_file
